...
exports.matchChoices = function (message) {
    var msg = message || 'Invalid value';
    return function (form, field, callback) {
        var isValid = field.choices.filter(choice => field.value === choice._value);
        if (isValid.length > 0) {
            callback();
        } else {
            callback(msg);
...
exports.requiresFieldIfEmpty = function (alternate_field, message) {
    ...
    var validator = function (form, field, callback) {
        var alternateBlank = String.prototype.trim(form.fields[alternate_field].value || '').length === 0;
        var fieldBlank = String.prototype.trim(field.value || '').length === 0;
...
exports.minlength = function (val, message) {
    ...
    return function (form, field, callback) {
        if (String(field.value).length >= val) {
            callback();
        } else {
            callback(format(msg, val));
        }
...
exports.rangelength = function (min, max, message) {
    ...
    return function (form, field, callback) {
        if (String(field.value).length >= min && String(field.value).length <= max) {
            callback();
        } else {
            callback(format(msg, min, max));
        }
